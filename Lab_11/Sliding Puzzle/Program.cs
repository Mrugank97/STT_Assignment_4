...
try
{
	while (true)  //Breakpoint 1: Entry Point of the Program
	{
	Menu:
		Console.Clear();
		Console.Write(menu);
		...
		while (board is null)
		{
			...
			switch (Console.ReadKey(true).Key)      // Breakpoint 2: Puzzle Size Selection
			{
		...
		Console.Clear();
		while (!IsSolved(board)) // Breakpoint 5: Win Check
		{
			Render(board);
			Console.Write(info);
			var space = FindFlatLength(board);
			Console.CursorVisible = false;
			switch (Console.ReadKey(true).Key)     // Breakpoint 4: Tile Movement (Game Logic)
			{
